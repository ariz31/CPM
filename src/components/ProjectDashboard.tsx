...
const WIDGET_CATALOG: WidgetCatalogItem[] = [
  ...
  { id: 'W-DURATION', title: 'Schedule duration', kind: 'metric', metric: 'schedule-duration', size: 'small', description: 'Calendar-aware project duration.' },
  ...
  { id: 'W-BAC', title: 'Budget at completion', kind: 'metric', metric: 'bac', size: 'small', description: 'Allocated budget at completion.' },
  { id: 'W-SPI', title: 'Schedule performance', kind: 'metric', metric: 'spi', size: 'small', description: 'Earned-value schedule performance index.' },
  { id: 'W-CPI', title: 'Cost performance', kind: 'metric', metric: 'cpi', size: 'small', description: 'Earned-value cost performance index.' },
  { id: 'W-FINDING-COUNT', title: 'Control findings', kind: 'metric', metric: 'control-findings', size: 'small', description: 'Schedule, allocation, date, and validation findings.' },
  { id: 'W-ALLOCATION', title: 'Budget allocation', kind: 'metric', metric: 'allocation-completeness', size: 'small', description: 'Share of estimate allocated to activities.' },
...
export function ProjectDashboard({ project, result, onReplace, onNavigate, onAddActivity, onCreateSnapshot }: ProjectDashboardProps) {
  ...
  return (
    <section className="dashboard-workspace" aria-labelledby="project-dashboard-title">
      <div className="dashboard-command-heading"><div><p className="eyebrow">Central project controls</p><h1 id="project-dashboard-title">Dashboard</h1><p className="muted">Choose the metrics, findings, charts, tables, and actions that matter for this project.</p></div><button className="button button-secondary" type="button" onClick={openConfigurator}>Customize dashboard</button></div>
      ...
      <div className="dashboard-grid">
        {dashboard.widgets.map((widget) => {
          ...
          if (kind === 'metric' && widget.metric) {
            ...
            return <div className={`dashboard-widget-host ${widget.size}`} key={widget.id}><MetricCard label={widget.title} value={formatDashboardValue(value?.value, value?.unit, project.metadata.currency)} detail={`${value?.unit ?? 'Unavailable'} · ${value?.completeness ?? 'unavailable'}`} tone={widget.metric === 'control-findings' && (value?.value ?? 0) > 0 ? 'warning' : widget.metric === 'cpi' && value?.value !== null && value?.value !== undefined && value.value < 1 ? 'critical' : 'default'} /></div>;
          }
...
function normalizeDashboard(stored: DashboardDefinition | undefined): DashboardDefinition {
  ...
  const normalizedLegacy = stored.widgets.map((widget) => ({ ...widget, kind: 'metric' as const }));
  const missingModern = fallback.widgets.filter((widget) => !normalizedLegacy.some((candidate) => candidate.id === widget.id));
  return { ...stored, widgets: [...normalizedLegacy, ...missingModern] };
}
