    ...
    <div className="activity-grid professional-activity-grid">
      ...
      >
        <div className="activity-grid-header" role="row" style={{ gridTemplateColumns }}>
          {visibleColumns.map((key, columnIndex) => {
            ...
            >
              ...
              {key !== 'select' ? <button className="column-resize-handle" type="button" onPointerDown={(event) => beginResize(event, key)} aria-label={`Resize ${definition.label} column`} /> : null}
            </span>;
          })}
        </div>
        ...
        {visibleRows.map((activity, visibleIndex) => {
          ...
          const rowIndex = absoluteIndex + 2;
            ...
            >
              {visibleColumns.map((key, columnIndex) => {
                ...
                return <span
                  ...
                  data-grid-cell={key}
                  key={key}
                  onPaste={(event) => handlePaste(event, absoluteIndex, key)}
                >
                  {renderCell(key, activity, calculated, status, progress[activity.id]?.percentComplete ?? 0, selectedIds, wbs, calendars, wbsById, calendarById, onToggle, updateCell)}
                </span>;
              })}
            </div>
          );
...
function renderCell(
  ...
  switch (key) {
    ...
    case 'duration': return <input className="grid-input duration-input" type="number" min={0} step={0.25} value={activity.duration} disabled={activity.type === 'milestone'} onChange={(event) => update(activity.id, key, { duration: Number(event.target.value) })} aria-label={`Duration for ${activity.id}`} />;
    ...
    default: return `${wbsById.get(activity.wbsId)?.code ?? ''} ${calendarById.get(activity.calendarId)?.name ?? ''}`;
  }
}
