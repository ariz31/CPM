  ...
  return (
    <section className="surface phase-surface" aria-labelledby="boq-title">
      ...
      <div className="boq-layout">
        <div className="report-table-scroll"><table className="report-table boq-table"><thead><tr><th>Code</th><th>Description</th><th>Unit</th><th>Quantity</th><th>Unit rate</th><th>Amount</th><th>Allocation</th><th /></tr></thead><tbody>{summary.items.map((calculated) => <tr key={calculated.id} className={selectedItemId === calculated.id ? 'selected-table-row' : ''}><td><input value={calculated.code} onChange={(event) => updateItem(calculated.id, { code: event.target.value })} /></td><td><input value={calculated.description} onChange={(event) => updateItem(calculated.id, { description: event.target.value })} /></td><td><input value={calculated.unit} onChange={(event) => updateItem(calculated.id, { unit: event.target.value })} /></td><td><input type="number" min={0} step="any" value={calculated.quantity} onChange={(event) => updateItem(calculated.id, { quantity: Number(event.target.value) })} /></td><td>{calculated.resources.length === 0 ? <input type="number" min={0} step="any" value={calculated.manualUnitRate ?? 0} onChange={(event) => updateItem(calculated.id, { manualUnitRate: Number(event.target.value) })} /> : money(calculated.unitRate, project.metadata.currency)}</td><td>{money(calculated.directAmount, project.metadata.currency)}</td><td><span className={`pill ${calculated.allocationStatus === 'balanced' ? '' : 'pill-warning'}`}>{calculated.allocationTotalPercent}% · {calculated.allocationStatus}</span></td><td><button className="button button-small" type="button" onClick={() => setSelectedItemId(calculated.id)}>Details</button></td></tr>)}</tbody></table></div>
      ...
      <section className="markup-section"><div className="surface-heading"><div><p className="eyebrow">Explicit calculation order</p><h3>Markup waterfall</h3></div></div>{summary.markups.length === 0 ? <p className="muted">No markups configured.</p> : <div className="markup-grid">{summary.markups.map((markup) => <article key={markup.id}><input value={markup.name} onChange={(event) => replaceBoq({ markups: project.boq.markups.map((item) => item.id === markup.id ? { ...item, name: event.target.value } : item) })} /><label>Rate %<input type="number" step="any" value={markup.ratePercent} onChange={(event) => replaceBoq({ markups: project.boq.markups.map((item) => item.id === markup.id ? { ...item, ratePercent: Number(event.target.value) } : item) })} /></label><label>Basis<select value={markup.basis} onChange={(event) => replaceBoq({ markups: project.boq.markups.map((item) => item.id === markup.id ? { ...item, basis: event.target.value as MarkupRule['basis'] } : item) })}><option value="direct-cost">Direct cost</option><option value="running-subtotal">Running subtotal</option></select></label><strong>{money(markup.amount, project.metadata.currency)}</strong></article>)}</div>}</section>
...
function BoqItemInspector({ project, item, onUpdate }: BoqItemInspectorProps) {
  function addResource(): void {
    onUpdate({ resources: [...item.resources, { id: crypto.randomUUID(), category: 'material', description: 'New resource', quantityPerUnit: 1, unit: item.unit, unitCost: 0, wastePercent: 0 }] });
  }
  ...
  return <><div className="surface-heading"><div><p className="eyebrow">Unit-price analysis</p><h3>{item.code}</h3></div></div><div className="inspector-actions"><button className="button button-small" type="button" onClick={addResource}>Add resource</button><button className="button button-small" type="button" onClick={addAllocation}>Add allocation</button></div><h4>Resources</h4>{item.resources.map((resource) => <div className="resource-row" key={resource.id}><select value={resource.category} onChange={(event) => onUpdate({ resources: item.resources.map((candidate) => candidate.id === resource.id ? { ...candidate, category: event.target.value as ResourceCategory } : candidate) })}><option value="material">Material</option><option value="labor">Labor</option><option value="equipment">Equipment</option><option value="subcontract">Subcontract</option><option value="miscellaneous">Misc.</option></select><input value={resource.description} onChange={(event) => onUpdate({ resources: item.resources.map((candidate) => candidate.id === resource.id ? { ...candidate, description: event.target.value } : candidate) })} /><label>Qty/unit<input type="number" min={0} step="any" value={resource.quantityPerUnit} onChange={(event) => onUpdate({ resources: item.resources.map((candidate) => candidate.id === resource.id ? { ...candidate, quantityPerUnit: Number(event.target.value) } : candidate) })} /></label><label>Unit cost<input type="number" min={0} step="any" value={resource.unitCost} onChange={(event) => onUpdate({ resources: item.resources.map((candidate) => candidate.id === resource.id ? { ...candidate, unitCost: Number(event.target.value) } : candidate) })} /></label><label>Waste %<input type="number" min={0} step="any" value={resource.wastePercent} onChange={(event) => onUpdate({ resources: item.resources.map((candidate) => candidate.id === resource.id ? { ...candidate, wastePercent: Number(event.target.value) } : candidate) })} /></label></div>)}<h4>Activity allocations</h4>{item.allocations.map((allocation) => <div className="allocation-row" key={allocation.activityId}><select value={allocation.activityId} onChange={(event) => onUpdate({ allocations: item.allocations.map((candidate) => candidate.activityId === allocation.activityId ? { ...candidate, activityId: event.target.value } : candidate) })}>{project.activities.map((activity) => <option key={activity.id} value={activity.id}>{activity.id} · {activity.name}</option>)}</select><input type="number" min={0} step="any" value={allocation.percent} onChange={(event) => onUpdate({ allocations: item.allocations.map((candidate) => candidate.activityId === allocation.activityId ? { ...candidate, percent: Number(event.target.value) } : candidate) })} /><span>%</span></div>)}</>;
}
