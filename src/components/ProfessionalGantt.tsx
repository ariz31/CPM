...
const ZOOM_PIXELS: Record<ZoomLevel, number> = { day: 42, week: 16, month: 6, quarter: 2.5 };
...
export function ProfessionalGantt({ project, result, selectedIds, onSelect }: ProfessionalGanttProps) {
  ...
  const pixelsPerDay = ZOOM_PIXELS[zoom];
  ...
  const rows = useMemo(() => [...(result?.activities ?? [])]
    .filter((activity) => !criticalOnly || activity.isCritical || activity.isNearCritical)
    .sort((left, right) => left.earlyStartOffsetDays - right.earlyStartOffsetDays || left.id.localeCompare(right.id)), [criticalOnly, result]);
  ...
  const width = LABEL_WIDTH + timelineDays * pixelsPerDay + 60;
  const height = 48 + rows.length * ROW_HEIGHT;
  const statusX = LABEL_WIDTH + (dateToOrdinal(project.statusDate) - startOrdinal) * pixelsPerDay;

  function jumpToStatusDate(): void {
    const container = scrollRef.current;
    if (!container) return;
    container.scrollLeft = Math.max(0, statusX - container.clientWidth / 2);
  }

  return (
    <section className={`surface phase-surface professional-gantt ${fullScreen ? 'gantt-full-screen' : ''}`} aria-labelledby="gantt-title">
      <div className="surface-heading phase-toolbar">
        ...
        <div className="toolbar-group wrap gantt-toolbar">
          <div className="segmented-control" aria-label="Gantt zoom level">{(['day', 'week', 'month', 'quarter'] as ZoomLevel[]).map((level) => <button key={level} type="button" className={zoom === level ? 'active' : ''} onClick={() => setZoom(level)} aria-pressed={zoom === level}>{level}</button>)}</div>
          <label><input type="checkbox" checked={showBaseline} onChange={(event) => setShowBaseline(event.target.checked)} /> Baseline</label>
          <label><input type="checkbox" checked={showProgress} onChange={(event) => setShowProgress(event.target.checked)} /> Progress</label>
          <label><input type="checkbox" checked={showFloat} onChange={(event) => setShowFloat(event.target.checked)} /> Float</label>
          <label><input type="checkbox" checked={criticalOnly} onChange={(event) => setCriticalOnly(event.target.checked)} /> Critical focus</label>
          <button className="button button-small" type="button" onClick={jumpToStatusDate}>Jump to status date</button>
          <button className="button button-small" type="button" onClick={() => setFullScreen((current) => !current)}>{fullScreen ? 'Exit full screen' : 'Full screen'}</button>
        </div>
      </div>
      {!result ? <div className="empty-state">A valid schedule calculation is required.</div> : (
        <div ref={scrollRef} className="gantt-scroll" tabIndex={0} aria-label="Scrollable Gantt chart">
          <svg width={width} height={height} role="group" aria-labelledby="gantt-title gantt-description">
            <desc id="gantt-description">Calendar-day Gantt with planned bars, baseline bars, progress, float, milestones, deadlines, critical filtering, zoom controls, and status date. Each activity row is keyboard operable.</desc>
            ...
            {Array.from({ length: timelineDays }, (_, day) => {
              ...
              const labelInterval = Math.max(1, Math.round(90 / pixelsPerDay));
              return <g key={day}><line x1={x} y1={28} x2={x} y2={height} className="gantt-grid-line" />{day % labelInterval === 0 ? <text x={x + 3} y={20} className="gantt-axis-label">D+{day}</text> : null}</g>;
            })}
      ...
      <div className="gantt-summary" role="status"><span>{rows.length} displayed activities</span><span>{result?.criticalActivityIds.length ?? 0} critical</span><span>{result?.nearCriticalActivityIds.length ?? 0} near critical</span><span>{zoom} scale</span></div>
      <details className="accessible-fallback"><summary>Accessible Gantt data table</summary><table><thead><tr><th>Activity</th><th>Early start</th><th>Early finish</th><th>Late finish</th><th>Float</th><th>Progress</th><th>Baseline finish</th></tr></thead><tbody>{rows.map((activity) => <tr key={activity.id}><td>{activity.id} — {activity.name}</td><td>{activity.earlyStartDate}</td><td>{activity.earlyFinishDate}</td><td>{activity.lateFinishDate}</td><td>{activity.totalFloat}</td><td>{project.progress[activity.id]?.percentComplete ?? 0}%</td><td>{baselineById.get(activity.id)?.plannedFinish ?? '—'}</td></tr>)}</tbody></table></details>
    </section>
  );
}
...
function GanttRow({ activity, baseline, progressPercent, selected, startOrdinal, pixelsPerDay, y, showBaseline, showProgress, showFloat, onSelect }: GanttRowProps) {
  ...
  return (
    <g className={selected ? 'gantt-row selected' : 'gantt-row'} onClick={onSelect} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect(); } }} aria-label={`Select ${activity.name}`}>
      ...
      <line x1={LABEL_WIDTH} y1={y + ROW_HEIGHT} x2={Math.max(lateFinish, finish) + 20} y2={y + ROW_HEIGHT} className="gantt-row-line" />
      ...
      {activity.type === 'milestone' ? <polygon points={`${start},${y + 17} ${start + 8},${y + 9} ${start + 16},${y + 17} ${start + 8},${y + 25}`} className={activity.isCritical ? 'gantt-milestone critical' : 'gantt-milestone'} /> : <><rect x={start} y={y + 12} width={barWidth} height={14} rx={4} className={activity.isCritical ? 'gantt-bar critical' : activity.isNearCritical ? 'gantt-bar near-critical' : 'gantt-bar'} />{showProgress ? <rect x={start} y={y + 12} width={barWidth * Math.max(0, Math.min(100, progressPercent)) / 100} height={14} rx={4} className="gantt-progress-bar" /> : null}</>}
