...
export function RiskResourcesPanel({ project, result, onReplace }: RiskResourcesPanelProps) {
  ...
  const productionActivities = useMemo(() => project.activities.filter((item) => item.type !== 'milestone'), [project.activities]);
  const [selectedActivity, setSelectedActivity] = useState(productionActivities[0]?.id ?? '');
  const effectiveSelectedActivity = productionActivities.some((activity) => activity.id === selectedActivity) ? selectedActivity : productionActivities[0]?.id ?? '';
    ...
    onReplace({
      ...
      riskResources: {
        ...
        risks: [...project.riskResources.risks, {
          ...
          impactDays: 1, impactCost: 0, owner: '', status: 'open', linkedActivityIds: effectiveSelectedActivity ? [effectiveSelectedActivity] : [], response: ''
        }]
      }
    });
  }
  ...
  function addFieldRecord(): void {
    if (!effectiveSelectedActivity) return;
    onReplace({
      ...project,
      riskResources: {
        ...project.riskResources,
        fieldRecords: [...project.riskResources.fieldRecords, {
          id: crypto.randomUUID(), activityId: effectiveSelectedActivity, date: project.statusDate,
          completedQuantity: 0, unit: normalizeEngineeringUnit(project.riskResources.productivityPlans.find((item) => item.activityId === effectiveSelectedActivity)?.unit ?? 'each'),
      ...
      <section className="surface">
        ...
        <div className="compact-table">
          {productionActivities.map((activity) => {
            ...
            return <div className="compact-row pert-row" key={activity.id}><strong className="activity-reference-id">{activity.id}</strong><span>{activity.name}</span>{(['optimistic', 'mostLikely', 'pessimistic'] as const).map((field) => <NumericInput key={field} min={0} step={0.25} calculatorLabel={`${field} duration for ${activity.name} (${activity.id})`} aria-label={`${field} duration for ${activity.name} (${activity.id})`} value={estimate?.[field]} placeholder={field === 'optimistic' ? 'O' : field === 'mostLikely' ? 'M' : 'P'} onValueChange={(value) => { if (value !== undefined) setPert(activity.id, field, value); }} />)}<span>μ {calculated?.expectedDuration ?? '—'}</span><span>σ² {calculated?.variance ?? '—'}</span></div>;
          })}
        </div>
      ...
      <div className="workspace-grid">
        <section className="surface">
          ...
          {analysis?.riskExposure.map((exposure) => {
            ...
            const linkedActivities = risk.linkedActivityIds.map((activityId) => activityReferenceFromId(project.activities, activityId));
            return <article className="risk-card" key={risk.id}><div><strong>{risk.title}</strong><span>{risk.status} · {risk.probabilityPercent}%</span></div><p>{risk.response || 'No response recorded.'}</p><p className="linked-activity-list"><strong>Linked activities:</strong> {linkedActivities.length > 0 ? linkedActivities.join(', ') : 'None'}</p><footer><span>{project.metadata.currency} {exposure.expectedCostExposure.toLocaleString('en-US')} expected</span><span>{exposure.expectedScheduleExposureDays} days expected</span></footer></article>;
          })}
          {project.riskResources.risks.length === 0 ? <p className="empty-state">No risks recorded.</p> : null}
        </section>

        <section className="surface">
          <div className="surface-heading"><div><p className="eyebrow">Field production</p><h2>Productivity forecast</h2></div><div className="toolbar-group"><label className="productivity-activity-picker">Activity<select value={effectiveSelectedActivity} onChange={(event) => setSelectedActivity(event.target.value)} aria-label="Productivity activity">{productionActivities.map((item) => <option key={item.id} value={item.id}>{activityReferenceLabel(item)}</option>)}</select></label><button className="button button-small" type="button" onClick={addFieldRecord} disabled={!effectiveSelectedActivity}>Add daily record</button></div></div>
          <div className="compact-table">{analysis?.productivity.map((item) => {
            ...
            return <div className="compact-row wide" key={item.planId}><strong>{activityReferenceFromId(project.activities, item.activityId)}</strong><span>Planned {item.plannedRatePerDay} {unit}/day</span><span>Actual {item.actualRatePerDay ?? 'Undefined'} {unit}/day</span><span>Remaining {item.remainingQuantity} {unit}</span><span>Forecast {item.forecastDaysRemaining ?? 'Undefined'} days</span></div>;
          })}</div>
          {project.riskResources.productivityPlans.length === 0 ? <button className="button button-secondary" type="button" disabled={!effectiveSelectedActivity} onClick={() => onReplace({ ...project, riskResources: { ...project.riskResources, productivityPlans: [...project.riskResources.productivityPlans, { id: crypto.randomUUID(), activityId: effectiveSelectedActivity, description: 'Planned production', quantity: 100, unit: 'each', plannedRatePerDay: 10 }] } })}>Create productivity plan for selected activity</button> : null}
        </section>
      </div>
