...
import { createProjectSnapshot, listJournal, listProjectSnapshots, restoreProjectSnapshot, saveProject } from '../infrastructure/projectRepository';
...
interface ScheduleWorkspaceProps { project: ProjectRecord; onBack: () => void; onProjectChange: (project: ProjectRecord) => void; }
...
export function ScheduleWorkspace({ project, onBack, onProjectChange }: ScheduleWorkspaceProps) {
  ...
  useEffect(() => { try { localStorage.setItem(SIDEBAR_STORAGE_KEY, sidebarMode); } catch { /* Preferences remain optional. */ } if (sidebarMode !== 'hidden') setLastVisibleSidebarMode(sidebarMode); }, [sidebarMode]);
  useEffect(() => {
    const request = calculateScheduleInWorker({ projectStartDate: project.metadata.startDate, defaultCalendarId: project.settings.defaultCalendarId, criticalFloatThresholdDays: project.settings.criticalFloatThresholdDays, nearCriticalFloatThresholdDays: project.settings.nearCriticalFloatThresholdDays, calendars: project.calendars, activities: project.activities, relationships: project.relationships }, project.revision);
    let active = true; setIsCalculating(true); setCalculationError(undefined);
    void request.result.then((nextResult) => { if (active) setResult(nextResult); }).catch((error: unknown) => { if (active && !String(error).includes('cancelled')) setCalculationError(error instanceof Error ? error.message : 'Unable to calculate schedule.'); }).finally(() => { if (active) setIsCalculating(false); });
    return () => { active = false; request.cancel(); };
  }, [project]);
  useEffect(() => { void refreshRecovery(); }, [project.id, project.revision]);

  async function refreshRecovery(): Promise<void> { const [nextSnapshots, nextJournal] = await Promise.all([listProjectSnapshots(project.id), listJournal(project.id)]); setSnapshots(nextSnapshots); setJournal(nextJournal); }
  async function applyCommand(command: ProjectCommand, historyMode: 'normal' | 'undo' | 'redo' = 'normal'): Promise<void> {
    setInteractionError(undefined);
    try {
      const commandResult = executeProjectCommand(projectRef.current, command); projectRef.current = commandResult.project; onProjectChange(commandResult.project); setSaveState('saving');
      const saved = await saveProject(commandResult.project, command.type, commandResult.summary, commandResult.commandId); projectRef.current = saved; onProjectChange(saved); setSaveState('saved');
      if (historyMode === 'normal') { setUndoStack((stack) => [...stack.slice(-49), commandResult.inverse]); setRedoStack([]); }
    ...
    } catch (error) { setSaveState('failed'); setInteractionError(error instanceof Error ? error.message : 'Unable to apply project change.'); }
  }
  async function undo(): Promise<void> { const command = undoStack.at(-1); if (!command) return; setUndoStack((stack) => stack.slice(0, -1)); await applyCommand(command, 'undo'); }
  async function redo(): Promise<void> { const command = redoStack.at(-1); if (!command) return; setRedoStack((stack) => stack.slice(0, -1)); await applyCommand(command, 'redo'); }
  function toggleSelection(activityId: string): void { setSelectedIds((current) => { const next = new Set(current); if (next.has(activityId)) next.delete(activityId); else next.add(activityId); return next; }); }
  async function deleteSelectedActivities(): Promise<void> { deleteDialogRef.current?.close(); for (const id of selectedIds) await applyCommand({ type: 'DELETE_ACTIVITY', activityId: id }); setSelectedIds(new Set()); }
  function addDictionaryActivities(activities: Partial<Activity>[]): void { void applyCommand({ type: 'ADD_ACTIVITIES', activities }); }
  function openSnapshotDialog(): void { setSnapshotName(`Snapshot ${new Date().toLocaleString('en-US')}`); snapshotDialogRef.current?.showModal(); }
  async function createNamedSnapshot(): Promise<void> { const normalized = snapshotName.trim(); if (!normalized) return; snapshotDialogRef.current?.close(); await createProjectSnapshot(projectRef.current, normalized); setSnapshotName(''); await refreshRecovery(); }
  function toggleSidebarVisibility(): void { setSidebarMode((current) => current === 'hidden' ? lastVisibleSidebarMode : 'hidden'); }
  ...
  return <main className="workspace-shell modern-workspace">
    <header className="workspace-header compact-project-context"><div className="workspace-title-group compact"><button className="back-button" type="button" onClick={onBack} aria-label="Return to project library">←</button><div><div className="workspace-breadcrumb"><span>Projects</span><span aria-hidden="true">/</span><strong>{TAB_LABELS[tab]}</strong></div><h1>{project.name}</h1><span className="workspace-subtitle">Status {project.statusDate} · Revision {project.revision} · {project.activities.length} activities</span></div></div><div className="workspace-header-status"><span className={`save-indicator save-${saveState}`} role="status"><span aria-hidden="true" />{saveState === 'saved' ? 'Saved locally' : saveState === 'saving' ? 'Saving…' : 'Save failed'}</span><button className="button button-small workspace-navigation-toggle" type="button" aria-expanded={sidebarMode !== 'hidden'} aria-label={sidebarMode === 'hidden' ? 'Show project navigation' : 'Hide project navigation'} onClick={toggleSidebarVisibility}><span aria-hidden="true">☰</span><span>{sidebarMode === 'hidden' ? 'Show navigation' : 'Hide navigation'}</span></button><WorkspaceFullscreenToggle /><button className="button button-small workspace-undo" type="button" onClick={() => void undo()} disabled={undoStack.length === 0}>Undo</button><button className="button button-small workspace-redo" type="button" onClick={() => void redo()} disabled={redoStack.length === 0}>Redo</button></div></header>
    <div className={`workspace-frame sidebar-${sidebarMode}`}><WorkspaceNavigation active={tab} onChange={setTab} sidebarMode={sidebarMode} onSidebarModeChange={setSidebarMode} /><section className="workspace-main" aria-label="Selected project workspace">
      {isCalculating ? <div className="notice workspace-calculation-notice" role="status">Recalculating in a dedicated worker…</div> : null}{calculationError ? <div className="notice notice-error" role="alert">{calculationError}</div> : null}{interactionError ? <div className="notice notice-error" role="alert">{interactionError}</div> : null}
      ...
      {tab === 'progress' ? <BaselineProgressPanel project={project} result={result} onReplace={(next) => void applyCommand({ type: 'REPLACE_PROJECT', project: next })} /> : null}{tab === 'boq' ? <BoqWorkspace project={project} onReplace={(next) => void applyCommand({ type: 'REPLACE_PROJECT', project: next })} /> : null}{tab === 'controls' ? <CostControlPanel project={project} result={result} onReplace={(next) => void applyCommand({ type: 'REPLACE_PROJECT', project: next })} /> : null}{tab === 'risk' ? <RiskResourcesPanel project={project} result={result} onReplace={(next) => void applyCommand({ type: 'REPLACE_PROJECT', project: next })} /> : null}{tab === 'reports' ? <ScheduleReportsPanel project={project} result={result} /> : null}{tab === 'enterprise' ? <EnterprisePanel project={project} result={result} journal={journal} onReplace={(next) => void applyCommand({ type: 'REPLACE_PROJECT', project: next })} /> : null}{tab === 'project' ? <ProjectSettingsPanel project={project} onChange={(changes) => void applyCommand({ type: 'REPLACE_PROJECT', project: { ...projectRef.current, ...changes } })} /> : null}{tab === 'recovery' ? <RecoveryCenter snapshots={snapshots} journal={journal} onCreateSnapshot={openSnapshotDialog} onRestoreSnapshot={(snapshotId) => void restoreProjectSnapshot(snapshotId).then((restored) => { onProjectChange(restored); setUndoStack([]); setRedoStack([]); })} /> : null}
    </section></div>
...
function readSidebarMode(): SidebarMode {
  ...
  try { const stored = localStorage.getItem(SIDEBAR_STORAGE_KEY); return stored === 'compact' || stored === 'hidden' || stored === 'expanded' ? stored : 'expanded'; }
