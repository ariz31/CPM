...
export function executeProjectCommand(project: ProjectRecord, command: ProjectCommand): CommandResult {
  ...
  switch (command.type) {
    ...
    case 'UPDATE_ACTIVITY': {
      ...
      if (command.changes.id !== undefined && command.changes.id !== existing.id) throw new Error('Activity IDs are stable and cannot be changed. Create a new activity instead.');
      const updated: Activity = {
        ...existing,
        ...command.changes,
        id: existing.id,
        duration: command.changes.type === 'milestone' ? 0 : command.changes.duration ?? existing.duration,
        audit: { createdAt: existing.audit?.createdAt ?? now, updatedAt: now, source: existing.audit?.source ?? 'manual' }
      };
      next = { ...project, activities: project.activities.map((item) => item.id === existing.id ? updated : item) };
    ...
    case 'DELETE_ACTIVITY': {
      ...
      const removedRelationships = project.relationships.filter((item) => item.predecessorId === existing.id || item.successorId === existing.id);
      const progress = { ...project.progress };
      delete progress[existing.id];
      next = {
        ...project,
        activities: project.activities.filter((item) => item.id !== existing.id),
        relationships: project.relationships.filter((item) => item.predecessorId !== existing.id && item.successorId !== existing.id),
        progress,
        boq: {
          ...project.boq,
          items: project.boq.items.map((item) => ({ ...item, allocations: item.allocations.filter((allocation) => allocation.activityId !== existing.id) }))
        },
        controls: {
          ...project.controls,
          activityLoadings: project.controls.activityLoadings.filter((loading) => loading.activityId !== existing.id),
          actualCosts: project.controls.actualCosts.map((record) => record.activityId === existing.id ? { ...record, activityId: undefined, description: `${record.description} [activity ${existing.id} deleted]` } : record)
        },
        riskResources: {
          ...project.riskResources,
          pertEstimates: project.riskResources.pertEstimates.filter((estimate) => estimate.activityId !== existing.id),
          risks: project.riskResources.risks.map((risk) => ({ ...risk, linkedActivityIds: risk.linkedActivityIds.filter((id) => id !== existing.id) })),
          productivityPlans: project.riskResources.productivityPlans.filter((plan) => plan.activityId !== existing.id),
          fieldRecords: project.riskResources.fieldRecords.filter((record) => record.activityId !== existing.id),
          assignments: project.riskResources.assignments.filter((assignment) => assignment.activityId !== existing.id)
        }
      };
      inverse = { type: 'ADD_ACTIVITY', activity: existing, commandId };
      summary = `Deleted activity ${existing.id}, ${removedRelationships.length} links, and all live cross-module references; immutable snapshots remain unchanged`;
      break;
    }
    case 'BULK_UPDATE_ACTIVITIES': {
      ...
      next = {
        ...project,
        activities: project.activities.map((item) => selected.has(item.id) ? {
          ...item,
          ...command.changes,
          id: item.id,
          duration: command.changes.type === 'milestone' ? 0 : command.changes.duration ?? item.duration,
          audit: { createdAt: item.audit?.createdAt ?? now, updatedAt: now, source: item.audit?.source ?? 'manual' }
        } : item)
      };
      inverse = { type: 'BULK_UPDATE_ACTIVITIES', activityIds: before.map((item) => item.id), changes: {}, commandId };
    ...
    case 'ADD_RELATIONSHIP': {
      ...
      if (project.relationships.some((item) => item.predecessorId === command.relationship.predecessorId && item.successorId === command.relationship.successorId && item.type === command.relationship.type && item.lag === command.relationship.lag)) throw new Error('An identical relationship already exists.');
    ...
    case 'ADD_WBS': {
      if (project.wbs.some((item) => item.id === command.node.id || item.code === command.node.code)) throw new Error('WBS ID and code must be unique.');
    ...
    case 'DELETE_WBS': {
      ...
      next = {
        ...
        activities: project.activities.map((item) => item.wbsId === command.nodeId ? { ...item, wbsId: command.reassignTo } : item)
      };
    ...
    case 'UPDATE_CALENDAR': {
      ...
      next = { ...project, calendars: project.calendars.map((item) => item.id === existing.id ? updated : item) };
  ...
  if (command.type !== 'REPLACE_PROJECT') inverse = { type: 'REPLACE_PROJECT', project: structuredClone(project), commandId };
  const candidate: ProjectRecord = { ...next, revision: project.revision + 1, updatedAt: now };
