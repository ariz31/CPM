...
const activity = (id: string, wbsId: string, duration: number): Activity => ({
  id,
  name: id,
  duration,
  type: 'task',
...
describe('WBS tree model', () => {
  it('builds hierarchy rows with descendant activity, duration, and budget rollups', () => {
    const rows = buildWbsTreeRows({
      nodes,
      activities: [activity('A-1', 'substructure', 4), activity('A-2', 'superstructure', 6)],
      calculatedActivities: [],
      budgetByActivityId: new Map([['A-1', 1000], ['A-2', 2500]])
    });
  ...
  it('supports non-drag indent and outdent operations', () => {
    ...
    const outdented = outdentWbsNode(indented, 'superstructure');
    expect(outdented.find((node) => node.id === 'superstructure')?.parentId).toBe('root');
  });
