...
export function filterAndSortActivities(
  ...
  return activities
    .filter((activity) => {
      ...
      return `${activity.id} ${activity.name} ${activity.code ?? ''} ${wbsCodes.get(activity.wbsId) ?? ''} ${activity.notes ?? ''}`.toLowerCase().includes(normalized);
    })
    .slice()
    .sort((left, right) => {
...
function assignClipboardValue(changes: Partial<Activity>, key: ActivityColumnKey, raw: string, existing: Activity): void {
  switch (key) {
    case 'name':
      if (raw) changes.name = raw;
      break;
    case 'wbs':
      if (raw) changes.wbsId = raw;
      break;
    case 'calendar':
      if (raw) changes.calendarId = raw;
      break;
    case 'type':
    ...
    case 'deadline':
      changes.deadline = raw || undefined;
      break;
    default:
      break;
  }
