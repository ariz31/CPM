...
import { createFillDownUpdates, filterAndSortActivities, normalizeActivityColumnKeys, parseClipboardUpdates } from './activityWorkspace';

const activity = (id: string, name: string, duration: number, wbsId = 'wbs-1'): Activity => ({
  id, name, duration, type: 'task', wbsId, calendarId: 'calendar-1',
...
describe('activity workspace model', () => {
  ...
  it('parses spreadsheet clipboard rows into safe editable activity changes', () => {
    ...
    const updates = parseClipboardUpdates('Replacement name\t7\nSecond replacement\t9', ['select', 'id', 'name', 'duration'], 'name', activities, 0);
  ...
  it('creates deterministic fill-down changes from the first selected activity', () => {
    ...
    expect(createFillDownUpdates(activities, new Set(['A-1', 'A-2']), 'duration')).toEqual([{ activityId: 'A-2', changes: { duration: 2 } }]);
  });
});
